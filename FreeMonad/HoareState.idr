...
data Predicate : Type where 
  ...
  Exists : {s : Type} -> (s -> Predicate) -> Predicate
...
asType (p :=> q)  = asType p -> asType q
asType (a =:= b)  = a = b
asType (Exists {s} p) = (s >< (\x => asType (p x)))
...
hgetP : (DecEq s, DecEq a) => 
        HoareStateP s s (\s => T) (\s1, (a, s2) => s1 =:= s2 /\ s2 =:= a)
hgetP = HSP $
  \(s ** _) =>
    ((s, s) ** (Refl, Refl))
    
hreturnP : (DecEq s, DecEq a) => 
           (x : a) -> HoareStateP s a (\_ => T) (\s1, (y, s2) => s1 =:= s2 /\ x =:= y)
hreturnP y = HSP $
  \(s ** _) =>
    ((y, s) ** (Refl, Refl)
  )
...
hbindP : {a : Type} -> {b : Type} -> {s : Type}
                    -> {p1 : Pre' s} -> {q1 : Post' s a}
                    -> {p2 : a -> Pre' s} -> {q2 : a -> Post' s b}
                    -> HoareStateP s a p1 q1
                    -> ((x : a) -> HoareStateP s b (p2 x) (q2 x))
                    -> HoareStateP s b
                         (\s1 => 
                           p1 s1 /\ (fa. (\x => fa. (\s2 => q1 s1 (x, s2) :=> p2 x s2 )))
                         )
                         (\s1, (x, s3) => 
                           ex. (\tup => q1 s1 tup /\ q2 (fst tup) (snd tup) (x, s3))
                         )
hbindP (HSP st) g = HSP $ 
  \(s1 ** (l, r)) => 
    case st (s1 ** l) of 
